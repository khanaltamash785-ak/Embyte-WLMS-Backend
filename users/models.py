...
class WpPpressSessions(models.Model):
    session_id = models.BigAutoField(primary_key=True)  # <-- Set primary_key=True here
...
class OneTimeVideoToken(models.Model):
    ...
    user_phone = models.CharField(max_length=20, default='')  # Add default
    created_at = models.DateTimeField(auto_now_add=True)
    expires_at = models.DateTimeField()
    is_used = models.BooleanField(default=False)
    used_at = models.DateTimeField(null=True, blank=True)  # This was accessed_at before
...
class OneTimeReWatchToken(models.Model):
    """One-time token for rewatch templates"""
    token = models.UUIDField(default=uuid.uuid4, unique=True)
    user_id = models.IntegerField()
    message_id = models.CharField(max_length=50)  # e.g., 'm-3'
    video_id = models.CharField(max_length=100, default='')
    video_caption = models.CharField(max_length=500, default='')
    is_used = models.BooleanField(default=False)
    used_at = models.DateTimeField(null=True, blank=True)
    created_at = models.DateTimeField(default=timezone.now)
    expires_at = models.DateTimeField()
    meta_data = models.TextField(blank=True, null=True)  # JSON data
    
    class Meta:
        db_table = 'one_time_rewatch_tokens'
        indexes = [
            models.Index(fields=['token']),
            models.Index(fields=['user_id', 'message_id']),
    ...
    def __str__(self):
        return f"ReWatchToken({self.token}) - User:{self.user_id} - {self.message_id}"
...
class CourseReminderSettings(models.Model):
    """Global reminder settings"""
    reminder_30_min_enabled = models.BooleanField(default=True)
    reminder_6_hour_enabled = models.BooleanField(default=True)
    max_6_hour_reminders = models.IntegerField(default=3)  # Max 3 reminders every 6 hours
    dnd_start_time = models.TimeField(default='20:00:00')  # 8 PM
    dnd_end_time = models.TimeField(default='08:00:00')    # 8 AM
    created_at = models.DateTimeField(auto_now_add=True)
...
class CourseReminderLog(models.Model):
    """Track all reminders sent to users"""
    REMINDER_TYPES = [
        ('30_min', '30 Minutes'),
        ('6_hour', '6 Hours'),
    ...
    STATUS_CHOICES = [
        ...
        ('sent', 'Sent'),
        ('failed', 'Failed'),
        ('skipped_dnd', 'Skipped - Do Not Disturb'),
        ('cancelled', 'Cancelled'),
    ]
    ...
    message_id = models.CharField(max_length=20, default='')
    ...
    reminder_count = models.IntegerField(default=0)  # For 6-hour reminders
    triggered_by = models.CharField(max_length=50, default='system')  # 'system' or 'admin_user_id'
    error_message = models.TextField(blank=True)
    meta_data = models.TextField(blank=True)  # JSON data for additional info
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def is_in_dnd_period(self):
        ...
        try:
            ...
            dnd_start = settings_obj.dnd_start_time
            dnd_end = settings_obj.dnd_end_time
            
            # Handle overnight DND period (8 PM to 8 AM)
            if dnd_start > dnd_end:  # Overnight period
    ...
    def get_next_available_time(self):
        """Get next available time outside DND period"""
        try:
            settings_obj = CourseReminderSettings.objects.first()
            if not settings_obj:
                return timezone.now()
            ...
            # If currently in DND, schedule for DND end time
            if self.is_in_dnd_period():
                next_day = now.date()
                if now.time() > dnd_end:
                    next_day = now.date() + timedelta(days=1)
                
                return timezone.datetime.combine(next_day, dnd_end).replace(tzinfo=now.tzinfo)
...
class UserReminderPreference(models.Model):
    ...
    custom_dnd_start = models.TimeField(null=True, blank=True)
    custom_dnd_end = models.TimeField(null=True, blank=True)
    timezone_offset = models.IntegerField(default=0)  # Offset in minutes from UTC
